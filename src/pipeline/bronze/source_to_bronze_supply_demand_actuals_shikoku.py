"""Shikoku supply_demand_actuals raw-to-bronze ingestion.

The source is a flat DATE,TIME,実績(万kW),供給力想定値(万kW) CSV --
build_schema_exprs() can cast it directly (unlike power_usage_hokuriku's
multi-section report, which needs a custom parser).
...
def extract_target_date_rows(df: pl.DataFrame, target_date: date) -> pl.DataFrame:
    ...
    Source DATE is '2026/01/01' (zero-padded); TIME is unpadded ('0:00'..
...
def run_source_to_bronze_supply_demand_actuals_shikoku(
    client: RustFSClient,
    bucket_name: str,
    object_key: str,
    target_date: date,
    ...
    logger.info(
        "Starting raw-to-bronze ingestion: company=shikoku, target_date=%s, s3://%s/%s",
        target_date,
        bucket_name,
        object_key,
    )
    ...
    if day_df.is_empty():
        logger.warning(
            "No rows found for target_date=%s in company=shikoku snapshot; "
            "nothing to ingest",
            target_date,
        )
        return 0

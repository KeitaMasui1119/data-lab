"""Shikoku supply_demand_actuals source-to-raw scraping and upload workflow.

Shikoku publishes a single DATE,TIME,実績(万kW),供給力想定値(万kW) CSV
covering one calendar year, growing by one day's rows daily:
  GET https://www.yonden.co.jp/nw/denkiyoho/csv/juyo_shikoku_{year}.csv
...
OBJECT_PREFIX = "raw/supply_demand_actuals/shikoku"
DATASET_NAME = "supply_demand_actuals_shikoku"
...
class ShikokuScrapedRawObject:
    """Represents a scraped Shikoku supply_demand_actuals CSV, ready for raw storage."""
...
class ShikokuSupplyDemandActualsScraper(BaseHttpScraper):
    """HTTP scraper for Shikoku's supply_demand_actuals year CSV.
    ...
    def scrape(self, year: int) -> ShikokuScrapedRawObject:
        response = self.fetch_response(datetime(year, 1, 1, tzinfo=UTC))
        file_name = FILE_NAME_TEMPLATE.format(year=year)
        return ShikokuScrapedRawObject(file_name=file_name, body=response.content)


@dataclass(frozen=True)
class ShikokuSnapshotResult:
    """Result of a Shikoku supply_demand_actuals raw snapshot attempt."""
...
def scrape_supply_demand_actuals_shikoku_raw(
    ...
    bucket_name: str,
    year: int,
) -> ShikokuSnapshotResult:
    """Download Shikoku's supply_demand_actuals year CSV and save a raw
    ...
    if manifest_bytes is not None:
        ...
        if previous_hash == sha256:
            logger.info(
                "Shikoku supply_demand_actuals snapshot unchanged "
                "(year=%s, sha256=%.8s), skipping save",
                year,
                sha256,
            )
            return ShikokuSnapshotResult(
                skipped=True,
                bucket_name=bucket_name,
                year=year,
                sha256=sha256,
    ...
    logger.info(
        "Shikoku supply_demand_actuals snapshot saved: year=%s, sha256=%.8s, prefix=%s",
        year,
        sha256,
        snapshot_prefix,
    )

"""Tohoku supply_demand_actuals source-to-raw scraping and upload workflow.

Tohoku publishes a single DATE,TIME,実績(万kW) CSV covering one calendar
year, growing by one day's rows daily:
  GET https://setsuden.nw.tohoku-epco.co.jp/common/demand/juyo_{year}_tohoku.csv
...
OBJECT_PREFIX = "raw/supply_demand_actuals/tohoku"
DATASET_NAME = "supply_demand_actuals_tohoku"
...
class TohokuScrapedRawObject:
    """Represents a scraped Tohoku supply_demand_actuals CSV, ready for raw storage."""
...
class TohokuSupplyDemandActualsScraper(BaseHttpScraper):
    """HTTP scraper for Tohoku's supply_demand_actuals year CSV.
    ...
    def scrape(self, year: int) -> TohokuScrapedRawObject:
        response = self.fetch_response(datetime(year, 1, 1, tzinfo=UTC))
        file_name = FILE_NAME_TEMPLATE.format(year=year)
        return TohokuScrapedRawObject(file_name=file_name, body=response.content)


@dataclass(frozen=True)
class TohokuSnapshotResult:
    """Result of a Tohoku supply_demand_actuals raw snapshot attempt."""
...
def scrape_supply_demand_actuals_tohoku_raw(
    ...
    bucket_name: str,
    year: int,
) -> TohokuSnapshotResult:
    """Download Tohoku's supply_demand_actuals year CSV and save a raw
    ...
    if manifest_bytes is not None:
        ...
        if previous_hash == sha256:
            logger.info(
                "Tohoku supply_demand_actuals snapshot unchanged "
                "(year=%s, sha256=%.8s), skipping save",
                year,
                sha256,
            )
            return TohokuSnapshotResult(
                skipped=True,
                bucket_name=bucket_name,
                year=year,
                sha256=sha256,
    ...
    logger.info(
        "Tohoku supply_demand_actuals snapshot saved: year=%s, sha256=%.8s, prefix=%s",
        year,
        sha256,
        snapshot_prefix,
    )

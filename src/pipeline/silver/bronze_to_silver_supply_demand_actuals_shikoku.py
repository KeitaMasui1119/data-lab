"""Transform Shikoku supply_demand_actuals from bronze into silver.
...
Shikoku's bronze also carries supply_capacity_forecast_10k_kw (the other
2 companies don't), so it is the only one of the 3 with a non-empty
EXTRA_COLUMNS.
"""
...
DEFAULT_SCHEMA_PATH = (
    "/workspace/configuration/iceberg/schema/silver/supply_demand_actuals_shikoku.csv"
)
...
EXTRA_COLUMNS: tuple[str, ...] = ("supply_capacity_forecast_10k_kw",)
...
def run_bronze_to_silver_supply_demand_actuals_shikoku(
    *,
    ...
    """Run the bronze-to-silver transform for Shikoku's supply_demand_actuals."""

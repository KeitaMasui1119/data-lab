"""Unit tests for the Chugoku supply_demand_actuals raw-to-bronze parsing helpers.
...
def test_parse_year_csv_skips_update_and_blank_line():
    df = parse_year_csv(CHUGOKU_TEXT)
...
def test_extract_target_date_rows_filters_and_normalizes_unpadded_date():
    ...
    result = extract_target_date_rows(df, date(2026, 4, 1))

    assert result.height == 2
    assert set(result["DATE"].to_list()) == {"2026-04-01"}

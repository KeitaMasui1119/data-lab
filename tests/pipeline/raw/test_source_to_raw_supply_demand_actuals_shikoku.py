"""Unit tests for the Shikoku supply_demand_actuals source-to-raw scraping workflow.
...
CSV_BODY = _CSV_TEXT.encode("cp932")
...
def test_scrape_returns_body_and_year_keyed_file_name():
    ...
    scraper = ShikokuSupplyDemandActualsScraper(session=session)

    scraped = scraper.scrape(2026)

    assert isinstance(scraped, ShikokuScrapedRawObject)
    assert scraped.file_name == "juyo_shikoku_2026.csv"
...
def _make_scraper(body: bytes = CSV_BODY) -> MagicMock:
    scraper = MagicMock()
    scraper.scrape.return_value = ShikokuScrapedRawObject(

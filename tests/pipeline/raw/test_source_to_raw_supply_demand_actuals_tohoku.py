"""Unit tests for the Tohoku supply_demand_actuals source-to-raw scraping workflow.
...
def test_scrape_returns_body_and_year_keyed_file_name():
    ...
    scraper = TohokuSupplyDemandActualsScraper(session=session)

    scraped = scraper.scrape(2026)

    assert isinstance(scraped, TohokuScrapedRawObject)
...
def _make_scraper(body: bytes = CSV_BODY) -> MagicMock:
    scraper = MagicMock()
    scraper.scrape.return_value = TohokuScrapedRawObject(
        file_name="juyo_2026_tohoku.csv",
        body=body,
    )
    return scraper

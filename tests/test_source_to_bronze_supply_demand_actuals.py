"""Unit tests for the supply_demand_actuals raw-to-bronze parsing helpers.
...
either. Correctness against real live data (all 3 companies, 2026-08-14)
...
def test_parse_year_csv_skips_update_line_only_for_tohoku():
    df = parse_year_csv(TOHOKU_TEXT, BRONZE_CONFIGS["tohoku"].header_skip_rows)

    assert df.columns == ["DATE", "TIME", "実績(万kW)"]
    assert df.height == 3


def test_parse_year_csv_skips_update_and_blank_line_for_chugoku():
    df = parse_year_csv(CHUGOKU_TEXT, BRONZE_CONFIGS["chugoku"].header_skip_rows)

    assert df.columns == ["DATE", "TIME", "実績(万kW)"]
    assert df.height == 2


def test_parse_year_csv_keeps_4_columns_for_shikoku():
    df = parse_year_csv(SHIKOKU_TEXT, BRONZE_CONFIGS["shikoku"].header_skip_rows)
